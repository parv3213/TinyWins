...
import { applyDailyScoreToHealth, calculateTreeHealth } from "@/lib/analytics";
import {
...
export default function DashboardPage() {
    ...
    const finalizeDateIfNeeded = useCallback(
        async (dateStr: string, log: DayLog, baseHealth: number, fallbackHabitCount: number) => {
    ...
    const xpToNext = getXpToNextLevel(treeLevel);
    const xpPct = xpToNext > 0 ? Math.min(100, Math.round((treeXp / xpToNext) * 100)) : 0;
    const treePhase = getTreePhaseFromLevel(treeLevel);
    const treePhaseLabel = getTreePhaseLabel(treePhase);
    const streakMultiplier = getStreakXpMultiplier(stats.currentStreak);
    const streakBoostPct = Math.max(0, Math.round((streakMultiplier - 1) * 100));
    const healthLabel =
        effectiveTreeHealth >= 80
            ? "Thriving"
            : effectiveTreeHealth >= 55
              ? "Growing"
              : effectiveTreeHealth >= 35
                ? "Struggling"
                : "Withering";
            ...
            <main className="flex flex-col gap-6">
                ...
                <section className="grid grid-cols-2 gap-4">
                    ...
                    <div className="card-flat p-4 flex flex-col items-center justify-center text-center">
                        ...
                        <div className="mt-1 text-xs font-medium text-[var(--muted-fg)]">{treePhaseLabel}</div>
                        <div className="w-full mt-2">
                            ...
                            <div className="mt-1 text-[11px] font-medium text-[var(--success)]">
                                Streak growth boost: +{streakBoostPct}% XP
                            </div>
                        </div>
                    </div>
                </section>
                    ...
                    {loading ? (
                        ...
                        <div className="card-flat p-8 text-center bg-[var(--card)]/50 border-dashed border-2">
                            ...
                            <p className="text-[var(--muted-fg)] text-sm mb-4">
                                Create a habit to start growing your tree.
                            </p>
