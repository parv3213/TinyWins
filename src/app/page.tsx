    ...
    <div className="min-h-screen relative overflow-hidden flex flex-col">
      ...
      <main className="flex-1 app-container flex flex-col pt-12 pb-24 text-center">
          ...
          <div className="relative max-w-sm mx-auto w-full transition-transform hover:scale-[1.02] duration-500 cursor-default">
             ...
             <TreeCanvas health={DEMO_HEALTH} />
             
             {/* Floating mock-habits */}
             <div className="absolute -right-6 top-1/4 bg-[var(--card)] p-2 rounded-xl border border-[var(--border)] shadow-md animate-slideUp flex items-center gap-2" style={{ animationDelay: '500ms' }}>
                <span className="w-6 h-6 flex justify-center items-center rounded-full bg-[var(--success-light)] text-xs">💧</span>
                <span className="text-xs font-medium">Water</span>
                <span className="text-[var(--success)] text-xs ml-1">✓</span>
             </div>
             
             <div className="absolute -left-6 bottom-1/3 bg-[var(--card)] p-2 rounded-xl border border-[var(--border)] shadow-md animate-slideUp flex items-center gap-2" style={{ animationDelay: '800ms' }}>
                <span className="w-6 h-6 flex justify-center items-center rounded-full bg-[var(--success-light)] text-xs">📖</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-24 text-left">
             <div className="card-flat bg-[var(--card)]/80 hover:bg-[var(--card)] transition-colors">
                ...
                <p className="text-sm text-[var(--muted-fg)]">Track positive routines and avoid negative ones. Takes less than 30 seconds a day.</p>
             </div>
             <div className="card-flat bg-[var(--card)]/80 hover:bg-[var(--card)] transition-colors">
                <div className="text-3xl mb-4">🌳</div>
                <h3 className="text-lg font-semibold mb-2">Visual Growth</h3>
                <p className="text-sm text-[var(--muted-fg)]">Your tree reflects your choices. Complete habits to see it flourish, ignore them and watch it decay.</p>
             </div>
             <div className="card-flat bg-[var(--card)]/80 hover:bg-[var(--card)] transition-colors">
                <div className="text-3xl mb-4">📊</div>
                <h3 className="text-lg font-semibold mb-2">Insightful Trends</h3>
                <p className="text-sm text-[var(--muted-fg)]">View Apple Fitness-style completion rings, monthly bars, and track your longest streaks.</p>
             </div>
          </div>
       </main>
